...
mongoose.connect(
  `mongodb+srv://zh1004:${password}@cluster1.ba0lgic.mongodb.net/laboutiquex`
);
...
// API for uploading picture of a product
app.use("/images", express.static("upload/images"));

app.post("/upload", upload.single("product"), (req, res) => {
  console.log(req.file); // Check if req.file exists
  res.json({
    success: "1",
...
app.post("/signup", async (req, res) => {
  ...
  if (check) {
    return res
      .status(400)
      .json({ success: false, errors: "Email address already registered." });
  }
  let cart = {};
  for (let i = 0; i < 300; i++) {
    cart[i] = 0;
...
// API endpoint for popular section
...
// API endpoint to remove products from cart
